...
int dtoa_grisu3(double v, char *dst,unsigned int dst_length)
{
	...
	// Prehandle NaNs
	if ((u64 << 1) > 0xFFE0000000000000ULL) return snprintf(dst,dst_length, "NaN(%08X%08X)", (uint32_t)(u64 >> 32), (uint32_t)u64);
	...
	if (!success) return snprintf(s2, dst_length,"%.17g", v) + (int)(s2 - dst);
